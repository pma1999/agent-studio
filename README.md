...
Al primer arranque con auth se crea un usuario administrador cuyo email está fijo en `server/db.ts`; su contraseña viene de `INITIAL_ADMIN_PASSWORD` (si no, una aleatoria se imprime **una vez en los logs**). Todos los datos se reasignan del usuario local al admin. Multi-arrendatario (multi-tenant) real: todas las tablas llevan `user_id` y toda consulta está acotada a su dueño; recursos ajenos devuelven 404 uniforme.
