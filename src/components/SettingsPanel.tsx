...
import { CheckCircle, AlertCircle, ExternalLink, Zap, Coins, BarChart3, Loader2, Globe, KeyRound, Database, MessageSquare, Brain, Check, ChevronDown, Sparkles, Lightbulb, SlidersHorizontal, Wrench, Plug, Layers, Link, Box, LogOut, Copy, RefreshCw, Cpu } from 'lucide-react';
...
import { DEEPSEEK_ACCENT, CODEX_ACCENT, LMSTUDIO_ACCENT, LMSTUDIO_STATUS_CHANGED_EVENT, isLmStudioModel, stripLmStudioPrefix } from '../utils/providers';
import { chatgptApi, lmstudioApi, type ChatgptStatus, type LmStudioStatus, type LmStudioProfileId } from '../api/client';
...
import { LmStudioCompliancePanel } from './LmStudioCompliancePanel';
...
// LM Studio (local provider) — settings keys and profile table per
// plans/lmstudio-local-provider/global-constraints.md §3/§9.
const LMSTUDIO_DEFAULT_BASE_URL = 'http://127.0.0.1:1234';
const LMSTUDIO_DEFAULT_PROFILE: LmStudioProfileId = 'equilibrado';

/** Shared advisory applied to EVERY profile; only context knobs are REST-settable. */
const LMSTUDIO_PROFILE_ADVISORY = 'Q4_K_M · flash attention ON · KV in RAM · batch 512';

const LMSTUDIO_PROFILES: { id: LmStudioProfileId; label: string; contextLength: number }[] = [
  { id: 'rapido', label: 'RÁPIDO', contextLength: 32768 },
  { id: 'equilibrado', label: 'EQUILIBRADO', contextLength: 65536 },
  { id: 'contexto_grande', label: 'CONTEXTO GRANDE', contextLength: 131072 },
];

/** A masked value (contains '****') is the server-masked saved token, never the real one. */
function isMaskedValue(v: string): boolean {
  return v.includes('****');
}

function isValidLmStudioBaseUrl(v: string): boolean {
  try {
    const url = new URL(v);
...
/**
 * LM Studio (local) provider section.
 *
 * Base URL + optional bearer token, connection test showing transport and API
 * surface, load-profile selector, and live compliance panels for currently
 * loaded models. Degrades gracefully when the local server is unreachable.
 */
function LmStudioSection() {
  const [baseUrl, setBaseUrl] = useState(LMSTUDIO_DEFAULT_BASE_URL);
  const [savedBaseUrl, setSavedBaseUrl] = useState<string | null>(null);
  const [baseUrlSaving, setBaseUrlSaving] = useState(false);
  const [profile, setProfile] = useState<LmStudioProfileId>(LMSTUDIO_DEFAULT_PROFILE);
  ...
  // §11 memory management (auto-swap + idle unload), optimistic-save idiom.
  const [autoSwap, setAutoSwap] = useState(true);
  const [idleMinutesText, setIdleMinutesText] = useState('45');
  const [idleMinutesSaved, setIdleMinutesSaved] = useState('45');

  // Catalog comes from the shared fail-soft hook; `loaded` flags mark live instances.
  const { models: lmStudioModels } = useLmStudioModels();
  const loadedModels = lmStudioModels.filter((m) => m.loaded === true && isLmStudioModel(m.id));

  useEffect(() => {
    let cancelled = false;
    Promise.all([
      settingsApi.get('lmstudio_base_url'),
      settingsApi.get('lmstudio_profile'),
      settingsApi.get('lmstudio_api_token'),
      settingsApi.get('lmstudio_auto_swap'),
      settingsApi.get('lmstudio_idle_unload_minutes'),
    ]).then(([base, prof, token, swap, idle]) => {
      if (cancelled) return;
      const savedBase = base.value || LMSTUDIO_DEFAULT_BASE_URL;
      setBaseUrl(savedBase);
      setSavedBaseUrl(savedBase);
      const savedProfile = prof.value as LmStudioProfileId;
      setProfile(
        LMSTUDIO_PROFILES.some((p) => p.id === savedProfile) ? savedProfile : LMSTUDIO_DEFAULT_PROFILE
      );
      // Sensitive keys come back masked ('****') — display-only until replaced.
      setTokenSaved(token.value ?? '');
      // §11 memory knobs: absent/invalid values fall back to the server defaults.
      setAutoSwap(swap.value !== 'false'); // anything but 'false' ⇒ enabled
      const parsedIdle = idle.value != null && idle.value.trim() !== '' ? Number(idle.value) : NaN;
      const normalizedIdle =
        Number.isFinite(parsedIdle) && parsedIdle >= 0 ? String(Math.floor(parsedIdle)) : '45';
      setIdleMinutesText(normalizedIdle);
      setIdleMinutesSaved(normalizedIdle);
    }).catch(() => {});
    return () => {
      cancelled = true;
    };
  ...
  const fireStatusChanged = () => window.dispatchEvent(new Event(LMSTUDIO_STATUS_CHANGED_EVENT));

  const baseUrlInvalid = !!baseUrl.trim() && !isValidLmStudioBaseUrl(baseUrl.trim());
  const handleSaveBaseUrl = async () => {
    if (!baseUrl.trim() || baseUrlInvalid || baseUrlSaving) return;
    setBaseUrlSaving(true);
    try {
      await settingsApi.set('lmstudio_base_url', baseUrl.trim());
      setSavedBaseUrl(baseUrl.trim());
      fireStatusChanged();
    } catch (err) {
      console.error('Failed to save LM Studio base URL:', err);
    } finally {
      setBaseUrlSaving(false);
    }
  };

  // Masked-save idiom (ProviderKeySection): never write a masked value back.
  const handleSaveToken = async () => {
    if (!tokenLocal || isMaskedValue(tokenLocal) || tokenSaving) return;
    setTokenSaving(true);
    try {
      const res = await settingsApi.set('lmstudio_api_token', tokenLocal);
      setTokenSaved((res as { value?: string }).value ?? tokenLocal);
      setTokenLocal('');
      fireStatusChanged();
    } catch (err) {
      console.error('Failed to save LM Studio API token:', err);
    } finally {
  ...
  const handleProfileChange = async (id: string) => {
    ...
    const next = id as LmStudioProfileId;
    if (next === previous) return;
    setProfile(next);
    try {
      await settingsApi.set('lmstudio_profile', next);
      fireStatusChanged();
    } catch (err) {
      console.error('Failed to save LM Studio profile:', err);
      setProfile(previous);
    }
  };

  // §11 memory controls — handleProfileChange's optimistic-save-with-rollback idiom.
  const handleAutoSwapChange = async (next: boolean) => {
    const previous = autoSwap;
    if (next === previous) return;
    setAutoSwap(next);
    try {
      await settingsApi.set('lmstudio_auto_swap', next ? 'true' : 'false');
    } catch (err) {
      console.error('Failed to save LM Studio auto-swap:', err);
      setAutoSwap(previous);
    }
  };

  const commitIdleMinutes = async () => {
    const parsed = Number(idleMinutesText.trim());
    // Invalid/negative input normalizes to the server default (45), mirroring §11.
    const normalized =
      Number.isFinite(parsed) && parsed >= 0 ? String(Math.floor(parsed)) : '45';
    setIdleMinutesText(normalized);
    if (normalized === idleMinutesSaved) return;
    const previousSaved = idleMinutesSaved;
    setIdleMinutesSaved(normalized); // optimistic
    try {
      await settingsApi.set('lmstudio_idle_unload_minutes', normalized);
    } catch (err) {
      console.error('Failed to save LM Studio idle unload minutes:', err);
  ...
  const handleTestConnection = async () => {
    ...
    setStatusError(null);
    try {
      const s = await lmstudioApi.status();
      setStatus(s);
      fireStatusChanged();
    } catch (err) {
      setStatus(null);
      setStatusError(err instanceof Error ? err.message : 'Could not check LM Studio status');
    } finally {
      setTesting(false);
    }
  };
  ...
  const transportBadge =
    status?.transport === 'direct' ? 'Direct' : status?.transport === 'relay' ? 'Relay' : '—';
  const apiSurfaceNote = !status?.apiSurface
    ? '—'
    : status.apiSurface === 'native-v1'
      ? 'native v1 (0.4+)'
      : 'OpenAI-compat only (≤0.3.x): profiles cannot be applied via REST';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{
          ...
          height: '28px',
          borderRadius: 'var(--radius-sm)',
          background: `${LMSTUDIO_ACCENT}18`,
          border: `1px solid ${LMSTUDIO_ACCENT}30`,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          color: LMSTUDIO_ACCENT,
        }}>
          <Cpu size={15} />
        </div>
        <h4 style={{
      ...
      <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
        Run models on your own machine through LM Studio — free, private, no cloud billing.
        Install a model, start the local server in LM Studio (Developer → Start Server), and its
        catalog appears in the model selector automatically.
      </p>
      ...
      <Input
        ...
        error={baseUrlInvalid ? 'Enter a valid URL starting with http:// or https://' : undefined}
        style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8125rem' }}
      />
      <div className="settings-provider-actions">
        <Button
          ...
          disabled={!baseUrl.trim() || baseUrlInvalid || baseUrl.trim() === savedBaseUrl}
        >
          Save URL
        </Button>
      </div>

      {/* Optional bearer token — masked-save idiom */}
      <Input
        label="API token (optional)"
        type="password"
        value={tokenLocal}
        onChange={(e) => setTokenLocal(e.target.value)}
        placeholder={tokenSaved ? `Saved: ${tokenSaved} — enter a new token to replace` : 'Optional bearer token'}
        style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8125rem' }}
      />
      <div className="settings-provider-actions">
        <Button
          ...
          loading={tokenSaving}
          disabled={!tokenLocal || isMaskedValue(tokenLocal)}
        >
          Save Key
        </Button>
      </div>
        ...
        {status?.reachable && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <span style={{ fontWeight: 600, color: 'var(--success)' }}>Reachable</span>
              <span style={{
                fontSize: '0.5625rem', padding: '1px 5px', background: `${LMSTUDIO_ACCENT}18`,
                color: LMSTUDIO_ACCENT, borderRadius: '4px', fontWeight: 600,
        ...
        {status && !status.reachable && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: 'var(--state-warning)', fontWeight: 600 }}>Unreachable</span>
            </div>
            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
              No LM Studio server at {status.baseUrl || savedBaseUrl || LMSTUDIO_DEFAULT_BASE_URL}.
              Start it in LM Studio (Developer → Start Server). Other providers are unaffected.
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <PremiumSelect
          ...
          options={LMSTUDIO_PROFILES.map((p) => ({
            value: p.id,
            label: p.label,
            description: `${Math.round(p.contextLength / 1024)}K context`,
            badge: p.id === LMSTUDIO_DEFAULT_PROFILE ? 'Default' : undefined,
          }))}
        />
        <div style={{ fontSize: '0.6875rem', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>
          {LMSTUDIO_PROFILE_ADVISORY}
        </div>
        <div style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
          Sampling defaults: general ≈0.65 temperature / coding ≈0.2–0.4, top_p 0.9 — per-agent
          override via agent temperature.
        </div>
      </div>

      {/* Memory management (§11): swap-on-load + idle auto-unload */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <label
          style={{
            display: 'flex',
        ...
        >
          <input
            type="checkbox"
            checked={autoSwap}
            onChange={(e) => void handleAutoSwapChange(e.target.checked)}
            style={{ width: '16px', height: '16px', accentColor: 'var(--accent)' }}
          />
          <span>Auto-swap models — loading one unloads others</span>
        </label>
        ...
        <div style={{ fontSize: '0.6875rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
          Loaded models share your LM Studio server's memory pool; unload actions act on the
          physically loaded instances regardless of which chat loaded them.
        </div>
      </div>

      {/* Compliance for currently loaded instances */}
      {loadedModels.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {loadedModels.map((m) => (
            <LmStudioCompliancePanel key={m.id} model={stripLmStudioPrefix(m.id)} name={formatModelId(m.id)} />
          ))}
        </div>
      )}
    </div>
