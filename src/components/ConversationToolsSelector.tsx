...
import { Wrench, Plug, Check, X, ChevronDown, RotateCcw, Loader2 } from 'lucide-react';
import { Badge } from './ui/Badge';
import { PremiumToggle } from './ui/PremiumToggle';
import { toolsApi, mcpServersApi } from '../api/client';
import { useIsMobile, usePrefersReducedMotion } from '../utils/breakpoints';
import type { Tool, McpServer } from '../types';

interface ConversationToolsSelectorProps {
  /** Effective (pre-check) tool ids right now — the conversation's own override if active, otherwise the agent's/general chat's current defaults. */
  toolIds: string[];
  /** Effective (pre-check) MCP server ids right now. */
  mcpServerIds: string[];
  /** Whether a conversation-level override is currently saved (gates "Reset to agent defaults"). */
  overrideActive: boolean;
  onApply: (toolIds: string[], mcpServerIds: string[]) => void;
  onReset: () => void;
  disabled?: boolean;
  compact?: boolean;
}
...
export function ConversationToolsSelector({
  toolIds,
  mcpServerIds,
  overrideActive,
  onApply,
  ...
  disabled = false,
  compact = false,
}: ConversationToolsSelectorProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [allTools, setAllTools] = useState<Tool[]>([]);
  const [allMcpServers, setAllMcpServers] = useState<McpServer[]>([]);
  const [listsLoaded, setListsLoaded] = useState(false);
  const [listsLoading, setListsLoading] = useState(false);
  const [draftToolIds, setDraftToolIds] = useState<string[]>(toolIds);
  const [draftMcpServerIds, setDraftMcpServerIds] = useState<string[]>(mcpServerIds);
  const wrapRef = useRef<HTMLDivElement>(null);
  const isMobile = useIsMobile();
  const prefersReducedMotion = usePrefersReducedMotion();

  const effectiveKey = useMemo(
    () => `${sortedKey(toolIds)}|${sortedKey(mcpServerIds)}`,
    [toolIds, mcpServerIds]
  );
  ...
  useEffect(() => {
    if (!isOpen) return;
    setDraftToolIds(toolIds);
    setDraftMcpServerIds(mcpServerIds);
    // Intentionally keyed on isOpen + the serialized effective set, not on the
    // toolIds/mcpServerIds array references (which churn every parent render).
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, effectiveKey]);

  // Lazy-fetch the tool/MCP catalogs once, on first open — the trigger is always
  // mounted but the lists are only needed once the panel is opened.
  useEffect(() => {
    if (!isOpen || listsLoaded || listsLoading) return;
    setListsLoading(true);
    Promise.all([toolsApi.list(), mcpServersApi.list()])
      .then(([tools, servers]) => {
        setAllTools(tools);
  ...
  const isDirty =
    sortedKey(draftToolIds) !== sortedKey(toolIds) ||
    sortedKey(draftMcpServerIds) !== sortedKey(mcpServerIds);

  const totalActive = toolIds.length + mcpServerIds.length;

  const toggleTool = useCallback((id: string) => {
    setDraftToolIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  }, []);
  const toggleMcpServer = useCallback((id: string) => {
    setDraftMcpServerIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  }, []);

  const handleApply = useCallback(() => {
    onApply(draftToolIds, draftMcpServerIds);
    setIsOpen(false);
  }, [draftToolIds, draftMcpServerIds, onApply]);
  ...
  const triggerButton = (
    <motion.button
      ...
      aria-label="Tools and MCP servers for this conversation"
    ...
    >
      <Wrench size={12} style={{ opacity: 0.8, flexShrink: 0 }} />
      <span style={{ fontWeight: overrideActive ? 500 : 400 }}>Tools</span>
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Wrench size={16} style={{ color: 'var(--accent)', opacity: 0.8 }} />
      ...
      <div style={{ flex: 1, overflow: 'auto', WebkitOverflowScrolling: 'touch', padding: 16 }}>
        {listsLoading && !listsLoaded ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontSize: '0.875rem' }}>Loading tools...</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {renderSection(
              <Wrench size={14} />,
              'Tools',
              allTools.map((t) => ({ id: t.id, label: t.name, description: t.type })),
              draftToolIds,
              toggleTool
            )}
            {renderSection(
              <Plug size={14} />,
              'MCP Servers',
              allMcpServers.map((m) => ({ id: m.id, label: m.name, description: m.transport })),
              draftMcpServerIds,
              toggleMcpServer
            )}
          </div>
  ...
  return (
    <div ref={wrapRef} className="conv-tools-selector" style={{ position: 'relative' }}>
      {triggerButton}
      ...
      <style>{`
        ...
        .conv-tools-selector button:focus-visible {
